            ...
            <div class="user-info">
                ...
                <?php
                    if (isset($_GET['error'])) {
                        $error = $_GET['error'];
                        if ($error === 'NoError') {
                            echo '<p style="color: green; font-size: 15px; margin: 0 0 8px 0;">Amount successfully debited.</p>';
                        }   
                    }
                ?>
            </div>
            <div class="debit">
                <form action="debitverif.php" method="post">
                    <label for="amount">Amount to be debited ( < Balance ) : </label>
                    <input type="number" name="amount" id="amount" >
                    <?php
                    if (isset($_GET['error'])) {
                        $error = $_GET['error'];
                        if ($error === 'InvalidAmount') {
                            echo '<p style="color: red; font-size: 13px; margin: 0;">Your balance should be <'.$account->c_balance.'. Please try again.</p>';
                        }   
                    }
                    ?>
